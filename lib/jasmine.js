    ...
    for (property in b) {
        ...
        if (!this.equals_(a[property], b[property], mismatchKeys, mismatchValues)) {
            mismatchValues.push("'" + property + "' was '"
                    ...
                    + "' in expected, but was '"
                    + (a[property] ? jasmine.util.htmlEscape(a[property].toString()) : a[property]) + "' in actual.");
        }
    }
...
jasmine.Matchers.prototype.toHaveBeenCalledWith = function() {
    ...
    this.message = function() {
        if (this.actual.callCount == 0) {
            ...
            return "Expected spy to have been called with " + jasmine.pp(expectedArgs) + " but was called with "
...
jasmine.Matchers.prototype.toThrow = function(expected) {
    ...
    if (exception) {
        result = (expected === jasmine.undefined || this.env.equals_(exception.message || exception, expected.message
                || expected));
    }

    this.message = function() {
        if (exception
                && (expected === jasmine.undefined || !this.env.equals_(exception.message || exception,
                        expected.message || expected))) {
            return [ "Expected function to throw", expected.message || expected, ", but it threw",
...
(function() {
    var functionNames = [ "reportRunnerStarting", "reportRunnerResults", "reportSuiteResults", "reportSpecStarting",
            "reportSpecResults", "log" ];
...
jasmine.WaitsForBlock.prototype.execute = function(onComplete) {
    ...
    if (latchFunctionResult) {
        ...
        var message = 'timed out after ' + self.timeout + ' msec waiting for '
                + (self.message || 'something to happen');
        self.spec.fail({
            name : 'timeout',
            message : message
        });
...
jasmine.FakeTimer.prototype.runFunctionsWithinRange = function(oldMillis, nowMillis) {
    ...
    for ( var timeoutKey in this.scheduledFunctions) {
        ...
        if (scheduledFunc != jasmine.undefined && scheduledFunc.runAtMillis >= oldMillis
